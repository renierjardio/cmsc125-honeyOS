...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if (
      (introDone && lowerCasePayload.includes("good day honey")) ||
      lowerCasePayload.includes("good morning honey") ||
  ...
  return (
    <div
      className={`${montserrat.className} min-h-screen flex items-center justify-center bg-purple-900 relative`}
    >
      {introDone && (
        <FaVolumeHigh
          className={"absolute z-20 top-10 left-10 text-white animate-pulse"}
          size={50}
        />
      )}
      <div
        className={`absolute z-20 top-[15%] text-white text-[30px] transition-opacity ${
          introDone && honeySaid ? "opacity-100" : "opacity-0"
        }`}
      >
        Welcome Honey!
      </div>
